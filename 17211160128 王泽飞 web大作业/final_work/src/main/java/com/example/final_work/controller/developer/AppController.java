package com.example.final_work.controller.developer;
...
public class AppController {
	...
	public String addVersionSave(AppVersion appVersion, HttpSession session, HttpServletRequest request,
		...
		if(!attach.isEmpty()){
			...
			if(prefix.equalsIgnoreCase("apk")){//apk文件命名：apk名称+版本号+.apk
				...
				downloadLink = request.getContextPath()+"/uploadfiles/"+oldFileName;
	...
	@RequestMapping(value="/devadd",method=RequestMethod.GET)
	public String adddev(@ModelAttribute("appInfo") AppInfo appInfo){
		return "developer/devreg";
	}

	@RequestMapping(value="/devuseraddsave",method= RequestMethod.POST)
	public String addSave( DevUser devUser, HttpSession session, HttpServletRequest request){

	    devUser.setCreatedBy(((DevUser)session.getAttribute(Constants.DEV_USER_SESSION)).getId());
        devUser.setCreationDate(new Date());
        devUser.setStatus("待审核");
        map2.clear();

        if (devUserService.insert(devUser)==1){
            return "developer/main";
        }
		return "redirect:/dev/flatform/app/devreg";
	}


	@RequestMapping(value="/devUsermodify")
	public String adddev(@ModelAttribute("appInfo") AppInfo appInfo,
                         DevUser devUser,HttpSession session,Model model){
        Integer id = ((DevUser) session.getAttribute(Constants.DEV_USER_SESSION)).getId();
        devUser = devUserService.getUser(id);
        DevUser devUser1=devUserService.getUser(devUser.getCreatedBy());
        String devName = devUser1.getDevName();
        model.addAttribute("devUser",devUser);
        session.setAttribute("devName",devName);
		return "developer/devUserModify";
	}

	@RequestMapping(value="/devUsermodifysave",method=RequestMethod.POST)
	public String modifyUserSave(DevUser devUser, HttpSession session,
								 HttpServletRequest request){
        devUser.setId(((DevUser) session.getAttribute(Constants.DEV_USER_SESSION)).getId());
        ...
        int i = devUserService.modify(devUser);
